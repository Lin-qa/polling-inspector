...
一个独立的接口巡检工具，按 xlsx 配置持续轮询接口，并通过企业微信或飞书群机器人通知异常和恢复。
...
- 同一轮 3 次都失败后，发送群机器人通知
- 已告警接口恢复成功后，发送恢复通知
- 程序启动后发送一次启动通知
- 单次执行完成后发送一条执行报告到群机器人
- 每天 18:00 发送前一天 18:00 至当天 18:00 的巡检日报
- 支持企业微信 webhook 和飞书 webhook，飞书可选签名密钥
...
实际使用时可以复制一份并改名为 `config/巡检配置.xlsx`，再填写真实接口和企业微信或飞书 webhook。真实配置文件不会提交到 git。
...
| 通知组 | 对应“通知配置”sheet |
| 前置请求 | 可选。填写“前置请求”sheet 中的前置名称，多个前置用逗号分隔，会按顺序执行并回填变量 |
...
用于登录、刷新 ticket、刷新 token、查询派生参数等场景。巡检接口行填写同名“前置请求”后，会先调用前置请求并把提取到的值写回运行时变量；同一个前置请求成功后会在本次程序运行中复用，不会每个接口都重复请求。需要多个前置时，在“巡检接口”的“前置请求”列填写 `登录刷新ticket,会员详情取memberId` 这种形式。

| 列名 | 说明 |
|---|---|
| 是否启用 | `是` 启用，`否` 跳过 |
| 前置名称 | 例如 `登录刷新ticket`，供“巡检接口”sheet 引用 |
| 请求方式 | GET、POST 等 |
| URL | 前置请求地址，支持 `${变量}` |
| 请求头JSON | JSON 对象，例如 `{"Content-Type":"application/json"}` |
| 请求参数 | JSON、`key=value&key2=value2` 或 `无` |
| 成功判断 | 与巡检接口相同，例如 `status=200` |
| 提取变量JSON | 变量名到响应字段路径的映射，例如 `{"ticketid":"result.ticketid"}`。如果字段值本身是 JSON 字符串，也支持继续取子字段 |
| 超时时间ms | 单次前置请求超时时间 |
...
|---|---|
| 通知组 | 与巡检接口中的通知组对应 |
| Webhook地址 | 企业微信或飞书群机器人 webhook。兼容旧列名 `企业微信Webhook` |
| 通知类型 | 可填 `企业微信` 或 `飞书`；不填时会根据 webhook 地址自动识别飞书 |
| 签名密钥 | 飞书机器人开启签名校验时填写；未开启则留空 |
| 是否@所有人 | `是` 则通知时 @all |
| 备注 | 说明 |
...
编辑 `config/巡检配置.xlsx`，填入真实接口和企业微信或飞书 webhook。
...
- 企业微信 webhook、飞书 webhook 和飞书签名密钥属于敏感配置，不要提交到 git
